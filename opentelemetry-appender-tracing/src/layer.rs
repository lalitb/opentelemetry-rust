...
impl<'a> tracing::field::Visit for EventVisitor<'a> {
    fn record_debug(&mut self, field: &tracing::field::Field, value: &dyn std::fmt::Debug) {
        if field.name() == "message" {
            self.log_record_builder.with_body(AnyValue::from(format!("{:?}", value)));
        } else {
            self.log_record_builder.with_attribute(field.name(), format!("{:?}", value));
        }
    }

    fn record_str(&mut self, field: &tracing_core::Field, value: &str) {
        self.log_record_builder.with_attribute(field.name(), value.to_owned());
